...
int main(int argc, char **argv)
{
    ...
    char name_l[32] = "reset_l.yuv";
    char name_r[32] = "reset_r.yuv";
    ...
    for (int i = 0; i < 3; i++) {
        ...
        if (spend_too_long_time) {
            sleep(1);   // long time handle syncframe
        }

        qbuf_syncframe(&config, syncframe);

        if (spend_too_long_time) {
            reset_buf(&config);//If you want get the new syncframe, buf your handle spent too much time, please use it.
        }
    }
